...
fn test_zero_capacities() {
    ...
    let s = HS::new(NoContext);
    assert_eq!(s.capacity(), 0);

    let s = HS::default();
    assert_eq!(s.capacity(), 0);

    let s = HS::with_hasher(NoContext, RandomState::new());
    assert_eq!(s.capacity(), 0);

    let s = HS::with_capacity(NoContext, 0);
    assert_eq!(s.capacity(), 0);

    let s = HS::with_capacity_and_hasher(NoContext, 0, RandomState::new());
    assert_eq!(s.capacity(), 0);

    let mut s = HS::new(NoContext);
    s.insert(1);
    s.insert(2);
    s.remove(&1);
    s.remove(&2);
...
#[test]
fn test_disjoint() {
    let mut xs = HashSet::new(NoContext);
    let mut ys = HashSet::new(NoContext);
...
fn test_subset_and_superset() {
    let mut a = HashSet::new(NoContext);
    assert!(a.insert(0));
    assert!(a.insert(5));
    assert!(a.insert(11));
    assert!(a.insert(7));

    let mut b = HashSet::new(NoContext);
    assert!(b.insert(0));
    assert!(b.insert(7));
    assert!(b.insert(19));
    assert!(b.insert(250));
...
fn test_move_iter() {
    let hs = {
        let mut hs = HashSet::new(NoContext);
...
fn test_eq() {
    ...
    let mut s1 = HashSet::new(NoContext);

    s1.insert(1);
    s1.insert(2);
    s1.insert(3);

    let mut s2 = HashSet::new(NoContext);
...
#[test]
fn test_show() {
    let mut set = HashSet::new(NoContext);
    let empty = HashSet::<i32>::new(NoContext);
...
fn test_trivial_drain() {
    let mut s = HashSet::<i32>::new(NoContext);
    for _ in s.drain() {}
    assert!(s.is_empty());
    drop(s);
...
fn test_replace() {
    ...
    let mut s = HashSet::new(NoContext);
...
fn const_with_hasher() {
    const X: HashSet<(), NoContext, ()> = HashSet::with_hasher(NoContext, ());
    assert_eq!(X.len(), 0);
}

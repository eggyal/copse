...
use crate::polyfill::assert_matches;
use crate::test_helpers::test_rng;
use contextual_core::NoContext;
use rand::Rng;
...
fn test_zero_capacities() {
    ...
    let m = HM::new(NoContext);
    assert_eq!(m.capacity(), 0);

    let m = HM::default();
    assert_eq!(m.capacity(), 0);

    let m = HM::with_hasher(NoContext, RandomState::new());
    assert_eq!(m.capacity(), 0);

    let m = HM::with_capacity(NoContext, 0);
    assert_eq!(m.capacity(), 0);

    let m = HM::with_capacity_and_hasher(NoContext, 0, RandomState::new());
    assert_eq!(m.capacity(), 0);

    let mut m = HM::new(NoContext);
    m.insert(1, 1);
    m.insert(2, 2);
    m.remove(&1);
    m.remove(&2);
...
fn test_create_capacity_zero() {
    let mut m = HashMap::with_capacity(NoContext, 0);
...
#[test]
fn test_insert() {
    let mut m = HashMap::new(NoContext);
    assert_eq!(m.len(), 0);
...
fn test_into_iter_drops() {
    ...
    let hm = {
        let mut hm = HashMap::new(NoContext);
...
fn test_empty_remove() {
    let mut m: HashMap<i32, bool> = HashMap::new(NoContext);
...
fn test_insert_conflicts() {
    let mut m = HashMap::with_capacity(NoContext, 4);
    assert!(m.insert(1, 2).is_none());
...
#[test]
fn test_eq() {
    let mut m1 = HashMap::new(NoContext);
    m1.insert(1, 2);
    m1.insert(2, 3);
    m1.insert(3, 4);

    let mut m2 = HashMap::new(NoContext);
    m2.insert(1, 2);
    m2.insert(2, 3);
...
#[test]
fn test_show() {
    let mut map = HashMap::new(NoContext);
    let empty: HashMap<i32, i32> = HashMap::new(NoContext);
...
#[test]
fn test_extend_ref() {
    let mut a = HashMap::new(NoContext);
    a.insert(1, "one");
    let mut b = HashMap::new(NoContext);
    b.insert(2, "two");
    b.insert(3, "three");
...
fn test_try_reserve() {
    let mut empty_bytes: HashMap<u8, u8> = HashMap::new(NoContext);
    ...
    if let Err(AllocError { .. }) = empty_bytes.try_reserve(MAX_USIZE / 16).map_err(|e| e.kind()) {
        ...
        let mut empty_bytes2: HashMap<u8, u8> = HashMap::new(NoContext);
        let _ = empty_bytes2.try_reserve(MAX_USIZE / 16);
        let mut empty_bytes3: HashMap<u8, u8> = HashMap::new(NoContext);
        let _ = empty_bytes3.try_reserve(MAX_USIZE / 16);
        let mut empty_bytes4: HashMap<u8, u8> = HashMap::new(NoContext);
        assert_matches!(
...
mod test_drain_filter {
    ...
    #[test]
    fn empty() {
        let mut map: HashMap<i32, i32> = HashMap::new(NoContext);
    ...
    fn consuming_nothing() {
        ...
        assert!(map.drain_filter(|_, _| false).eq_sorted(std::iter::empty()));
        assert_eq!(map.len(), 3);
    }
...
#[test]
#[cfg(feature = "const_collections_with_hasher")]
fn const_with_hasher() {
    const X: HashMap<(), (), NoContext, ()> = HashMap::with_hasher(NoContext, ());
    assert_eq!(X.len(), 0);
}

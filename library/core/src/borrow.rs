...
/// `ContextualBorrow<T, Context>`, providing a reference to a `T` in the
/// trait’s [`contextual_borrow`] method. A type is free to borrow as several
/// different types. If it wishes to mutably borrow as the type – allowing
/// the underlying data to be modified, it can additionally implement
/// [`ContextualBorrowMut<T, Context>`].
///
/// Further, when providing implementations for additional traits, it needs
/// to be considered whether they should behave identical to those of the
/// underlying type as a consequence of acting as a representation of that
/// underlying type. Generic code typically uses `ContextualBorrow<T, Context>`
/// when it relies on the identical behavior of these additional trait
/// implementations. These traits will likely appear as additional trait bounds.
///
/// In particular `ContextualEq`, `ContextualOrd` and `Hash` must be equivalent
/// for borrowed and owned values:
/// `x.contextual_borrow(context).contextual_eq(y.contextual_borrow(context), context)`
/// should give the same result as `x.contextual_eq(&y, context)`.
///
...
/// [`contextual_borrow`]: ContextualBorrow::contextual_borrow
///
/// # Examples
///
...
/// [`String`]: std::string::String
pub trait ContextualBorrow<Borrowed, Context = NoContext>
where
    Borrowed: ?Sized,
    Context: ?Sized,
{
    ...
    ///
    /// ```
    /// use contextual_core::{borrow::ContextualBorrow, NoContext};
    ///
    /// fn check<T: ContextualBorrow<str>>(s: T) {
    ///     assert_eq!("Hello", s.contextual_borrow(&NoContext));
    /// }
    ///
    ...
    /// check(s);
    /// ```
    fn contextual_borrow<'a>(&'a self, context: &'a Context) -> &'a Borrowed;
}
...
pub trait ContextualBorrowMut<Borrowed: ?Sized, Context: ?Sized = NoContext>:
    ContextualBorrow<Borrowed, Context>
{
    ...
    ///
    /// ```
    /// use contextual_core::{borrow::ContextualBorrowMut, NoContext};
    ///
    /// fn check<T: ContextualBorrowMut<[i32]>>(mut v: T) {
    ///     assert_eq!(&mut [1, 2, 3], v.contextual_borrow_mut(&NoContext));
    /// }
    ///
    /// let v = vec![1, 2, 3];
    ///
    /// check(v);
    /// ```
    fn contextual_borrow_mut<'a>(&'a mut self, context: &'a Context) -> &'a mut Borrowed;
}
